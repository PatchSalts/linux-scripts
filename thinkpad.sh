...
options	root=UUID=`findmnt -no UUID /mnt/` resume=`findmnt -no SOURCE /mnt/swapfile` resume_offset=`filefrag -v /mnt/swapfile | awk '$1=="0:" {print substr($4, 1, length($4)-2)}'` rw
EOF
